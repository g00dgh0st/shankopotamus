...
public class Water2D : MonoBehaviour {
	...
	public void CreateWaterPlane()
	{
		...
		if (createCollider)
		{
			if (!use2DPhysics && GetComponent<Collider>() == null)  // 3D Collisions
			{
			...
			else if (use2DPhysics && GetComponent<Collider2D>() == null)  //2D Collisions
			{
	...
	private void InitializeWater()
	{
		...
		instantiatedWaterSplash = tempWaterSplash.GetComponent<ParticleSystem>();
		tempWaterSplash = Instantiate(afterPeakwaterSplash) as GameObject;
		instantiatedAfterPeakwaterSplash = tempWaterSplash.GetComponent<ParticleSystem>();
	...
	void OnTriggerEnter(Collider _collider)  //3D collision
	{
		...
		Vector3 rigidBodySpeed = _collider.GetComponent<Rigidbody>().velocity;
	...
	void OnTriggerEnter2D(Collider2D _collider2D)
	{
		...
		//Get the regidbody speed
		Vector3 rigidBodySpeed = _collider2D.GetComponent<Rigidbody2D>().velocity;
		
		//We only use the vertical component
		
		ObjectEnteredWater(contactPoint, Mathf.Clamp(rigidBodySpeed.y, -maxWaterForceApplied,maxWaterForceApplied), _collider2D.GetComponent<Renderer>().bounds, true);
	}
